// Hook for discovering TV shows with search functionality
import { useState, useEffect, useCallback, useRef } from 'react';
import { Movie, SearchResponse } from '../types/movie';
import { tmdbApi } from '../utils/tmdbApi';

interface UseDiscoverTVShowsReturn {
  movies: Movie[];
  loading: boolean;
  error: string | null;
  currentPage: number;
...
export const useDiscoverTVShows = (): UseDiscoverTVShowsReturn => {
  ...
  const fetchMovies = useCallback(async (query: string, page: number = 1) => {
    ...
    try {
      ...
      if (query.trim()) {
        response = await tmdbApi.searchTVShows(query, page);
        setIsSearchMode(true);
      } else {
        response = await tmdbApi.discoverTVShows(page);
      ...
      const tvShowsWithType = filteredResults.map(show => ({
        ...show,
        media_type: 'tv' as const
      }));

      setMovies(tvShowsWithType);
